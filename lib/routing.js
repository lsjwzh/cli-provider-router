...
const DEFAULT_CODEX_SUBAGENT_PROVIDER = 'multicc_subagent';
...
function codexProviderProxyable(providerOrId, options = {}) {
  ...
  const proxyPath = options.codexProxyPath || '/codex-proxy/';
  return !!baseUrl && !new RegExp(`^https?://(?:127\\.0\\.0\\.1|localhost)(?::\\d+)?${proxyPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i').test(baseUrl);
}

function localProxyBase({ providerId, sessionId, role, port, proxyBaseUrl, codexProxyPath = '/codex-proxy' }) {
  const origin = (proxyBaseUrl || `http://127.0.0.1:${port}`).replace(/\/+$/, '');
  const prefix = '/' + String(codexProxyPath).replace(/^\/+|\/+$/g, '');
...
function materializeCodexRoutingHome(home, options) {
  ...
  config.model_providers[subagentProvider] = {
    name: options.subagentProviderLabel || 'MultiCC subagent route',
    base_url: localProxyBase({ ...routeOptions, providerId: subProviderId, sessionId, role: 'sub' }),
    wire_api: 'responses',
  ...
  for (const [role, meta] of Object.entries(roles)) {
    const agent = {
      ...
      model_provider: subagentProvider,
    };
...
function applyClaudeProxyEnv(env, options) {
  ...
  const proxyPath = '/' + String(options.claudeProxyPath || '/claude-proxy').replace(/^\/+|\/+$/g, '');
  env.ANTHROPIC_BASE_URL = `${origin}${proxyPath}/${encodeURIComponent(options.providerId)}/${encodeURIComponent(options.sessionId)}`;
  env.ANTHROPIC_AUTH_TOKEN = `${options.virtualTokenPrefix || 'multicc-'}${options.sessionId}`;
  delete env.ANTHROPIC_API_KEY;
  if (options.subagent && options.subagent.providerId && options.subagent.model) {
    env.CLAUDE_CODE_SUBAGENT_MODEL = `${options.modelPrefix || 'ccfw:'}${options.subagent.providerId}:${options.subagent.model}`;
  } else {
