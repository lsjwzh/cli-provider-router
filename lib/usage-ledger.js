...
function createUsageLedger(options = {}) {
  // A host-owned usage directory is a complete storage injection. Avoid
  // creating the standalone CPR tree simply to obtain unused default paths.
  const paths = options.paths
    ? ensureCprPaths(options.paths)
    : (options.usageDir ? null : ensureCprPaths(createCprPaths({ home: options.cprHome })));
  const usageDir = options.usageDir || paths.usageDir;
  const policyFile = options.policyFile || (paths
    ? paths.usagePolicyFile
    : path.join(path.dirname(usageDir), 'usage-policy.json'));
