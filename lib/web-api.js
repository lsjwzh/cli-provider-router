...
const SENSITIVE_KEY = /(?:token|secret|password|credential|api[_-]?key|auth)/i;
...
function usageFilters(query) {
  return Object.fromEntries(['from', 'to', 'cli', 'providerId', 'model', 'sessionId', 'role', 'limit']
...
async function queryUsage(ledger, filters, summary = false) {
  ...
  const method = summary
    ? (ledger.summary || ledger.getSummary)
  ...
  return { available: true, data: redact(await method.call(ledger, filters)) };
}
...
function createWebApp(options = {}) {
  ...
  app.get('/api/bootstrap', (req, res) => res.json({ adminToken, apiBase: '/api', capabilities: {
    providers: !!store, routeProfiles: !!profiles, ccSwitch: !!ccSwitch, usage: !!usageLedger, settings: !!settingsStore,
  } }));
  app.use('/api', adminGuard(adminToken));

  app.get('/api/dashboard', asyncRoute(async (_req, res) => {
    const [ccStatus, usage] = await Promise.all([
      ccSwitch && ccOptions.dbPath ? Promise.resolve(ccSwitch.status(ccOptions)).catch(error => ({ available: false, error: error.message })) : null,
  ...
  app.get('/api/ccswitch/detect', (_req, res) => res.json(redact(ccSwitch && ccOptions.dbPath ? ccSwitch.discover(ccOptions) : { found: false, supported: false, issues: ['not-configured'] })));
  app.get('/api/ccswitch/status', (_req, res) => res.json(redact(ccSwitch && ccOptions.dbPath ? ccSwitch.status(ccOptions) : { takeover: 'unavailable', managed: false })));
  app.post('/api/ccswitch/preview', (req, res) => {
    ...
    res.json(redact(ccSwitch.preview({ ...ccOptions, proxyBaseUrl, snapshotId: req.body && req.body.snapshotId })));
  });
  ...
  app.post('/api/ccswitch/apply', asyncRoute(async (req, res) => {
    ...
    res.json(redact(await ccSwitch.apply({ ...ccOptions, proxyBaseUrl, snapshotId: req.body.snapshotId })));
  }));
  ...
  app.use((error, _req, res, _next) => {
    const statusCode = Number(error.statusCode || error.status) || (error.code === 'CONFIG_DRIFT' ? 409 : 500);
    const body = { error: error.code || (statusCode >= 500 ? 'internal-error' : 'request-error'), message: statusCode >= 500 ? 'Request failed' : error.message };
