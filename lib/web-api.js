...
function profileToView(profile) {
  ...
  const routes = profile.cli === 'claude'
    ...
    : { main: profile.main || null, ...(profile.roles || {}) };
...
function viewToProfile(input, previous = null) {
  ...
  const allowed = cli === 'claude' ? new Set(['main', 'sub']) : null;
  const unknown = Object.keys(routes).filter(role => allowed ? !allowed.has(role) : !/^[a-z][a-z0-9_-]{0,63}$/i.test(role));
  ...
  return cli === 'claude'
    ...
    : { name: input.name, cli, enabled: input.enabled, main: routes.main || null, subagent: null,
        roles: Object.fromEntries(Object.entries(routes).filter(([role, endpoint]) => role !== 'main' && endpoint)) };
}

function requestError(message, code = 'VALIDATION_FAILED', statusCode = 400) {
  return Object.assign(new Error(message), { code, statusCode });
}

function validateModel(value, label = 'model') {
  if (value == null || value === '') return;
  const model = String(value).trim();
  if (!model || model.length > 200 || /[\u0000-\u001f]/.test(model)) throw requestError(`${label} is invalid`);
}

function validateHttpUrl(value, label = 'baseUrl') {
  if (value == null || value === '') return;
  let parsed;
  try { parsed = new URL(String(value)); } catch (_) { throw requestError(`${label} must be an absolute HTTP(S) URL`); }
  if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password || String(value).length > 2048) {
    throw requestError(`${label} must be an absolute HTTP(S) URL without embedded credentials`);
  }
}

function validateProviderInput(input = {}, appType = input.appType) {
  if (!['claude', 'codex'].includes(String(appType || '').toLowerCase())) throw requestError('appType must be claude or codex');
  if (input.name != null && !String(input.name).trim()) throw requestError('provider name is required');
  ...
  if (input.models != null) {
    ...
    if (models.length > 200) throw requestError('too many provider models');
    models.forEach((model, index) => validateModel(model, `models[${index}]`));
  }
  if (input.settingsConfig && typeof input.settingsConfig === 'object') {
    const cfg = parseConfig(input.settingsConfig);
    if (appType === 'claude') {
      validateHttpUrl(cfg.env && cfg.env.ANTHROPIC_BASE_URL, 'settingsConfig.env.ANTHROPIC_BASE_URL');
      validateModel(cfg.env && cfg.env.ANTHROPIC_MODEL, 'settingsConfig.env.ANTHROPIC_MODEL');
    } else {
      if (cfg.proxyTarget) validateHttpUrl(cfg.proxyTarget.baseUrl, 'settingsConfig.proxyTarget.baseUrl');
      let parsed = {};
      try { parsed = require('@iarna/toml').parse(cfg.config || ''); } catch (error) { throw requestError(`settingsConfig.config is invalid TOML: ${error.message}`); }
      const selected = parsed.model_provider && parsed.model_providers && parsed.model_providers[parsed.model_provider];
      if (selected) validateHttpUrl(selected.base_url, 'settingsConfig.config base_url');
      validateModel(parsed.model, 'settingsConfig.config model');
    }
  }
}

function validateRouteEndpoint(store, cli, endpoint, label) {
  if (!endpoint) return;
  const provider = store && typeof store.getProvider === 'function' ? store.getProvider(cli, endpoint.providerId) : null;
  if (!provider) throw requestError(`${label} references missing ${cli} provider: ${endpoint.providerId}`, 'ROUTE_PROVIDER_NOT_FOUND', 409);
  validateModel(endpoint.model, `${label}.model`);
  const summary = typeof store.getProviderSummary === 'function' ? store.getProviderSummary(cli, endpoint.providerId) : null;
  const models = summary && Array.isArray(summary.modelOptions) ? summary.modelOptions.map(String) : [];
  if (endpoint.model && models.length && !models.includes(String(endpoint.model))) {
    throw requestError(`${label}.model is not declared by provider ${endpoint.providerId}`, 'ROUTE_MODEL_NOT_FOUND', 409);
  }
  if (cli === 'codex') {
    const target = resolveProviderTarget(provider);
    if (target.error || !target.apiKey) throw requestError(`${label} provider is not proxyable: ${target.error || 'missing HTTP credential'}`, 'ROUTE_PROVIDER_NOT_PROXYABLE', 409);
  } else {
    const cfg = parseConfig(provider.settingsConfig);
    const baseUrl = cfg.env && cfg.env.ANTHROPIC_BASE_URL;
    const official = endpoint.providerId === 'claude-official';
    if (!baseUrl && !official) throw requestError(`${label} provider is not proxyable`, 'ROUTE_PROVIDER_NOT_PROXYABLE', 409);
    if (baseUrl) validateHttpUrl(baseUrl, `${label} provider base URL`);
  }
}

function validateRouteProfile(store, profile) {
  if (!profile || !profile.main) throw requestError('route profile main provider is required');
  validateRouteEndpoint(store, profile.cli, profile.main, 'main');
  validateRouteEndpoint(store, profile.cli, profile.subagent, 'subagent');
  Object.entries(profile.roles || {}).forEach(([role, endpoint]) => validateRouteEndpoint(store, profile.cli, endpoint, `roles.${role}`));
  return profile;
}

function providerReferences(profiles, appType, providerId) {
  if (!profiles || typeof profiles.list !== 'function') return [];
  return profiles.list().filter(profile => {
    if (profile.cli !== appType) return false;
    return [profile.main, profile.subagent, ...Object.values(profile.roles || {})].filter(Boolean).some(endpoint => endpoint.providerId === providerId);
  }).map(profile => ({ id: profile.id, name: profile.name }));
}

function activeProfile(directCliConfig, profileId) {
  if (!directCliConfig || typeof directCliConfig.status !== 'function') return null;
  for (const cli of ['claude', 'codex']) {
    const state = directCliConfig.status({ cli });
    if (state && state.active && state.profileId === profileId) return { cli, profileId };
  }
  return null;
}

function usageFilters(query) {
  const filters = Object.fromEntries(['from', 'to', 'cli', 'providerId', 'model', 'sessionId', 'role', 'roleKind', 'agentRole', 'routeName', 'status', 'coverage', 'limit']
...
function createWebApp(options = {}) {
  ...
  app.patch('/api/providers/:appType/:id', (req, res) => {
    ...
    validateProviderInput(req.body || {}, req.params.appType);
  ...
  app.delete('/api/providers/:appType/:id', (req, res) => {
    ...
    const references = providerReferences(profiles, req.params.appType, req.params.id);
    if (references.length) throw requestError(`provider is referenced by ${references.length} route profile(s)`, 'PROVIDER_IN_USE', 409);
  ...
  app.post('/api/routes', (req, res) => {
    ...
    const input = viewToProfile(req.body || {});
    validateRouteProfile(store, input);
    res.status(201).json({ profile: profileToView(profiles.create(input)) });
  });
  app.patch('/api/routes/:id', (req, res) => {
    ...
    if (activeProfile(directCliConfig, previous.id)) throw requestError('route profile is used by an active direct CLI takeover', 'ROUTE_PROFILE_IN_USE', 409);
    const priorView = profileToView(previous);
    const merged = { ...priorView, ...req.body, routes: { ...priorView.routes, ...(req.body.routes || {}) } };
    const input = viewToProfile(merged, previous);
    validateRouteProfile(store, input);
    res.json({ profile: profileToView(profiles.update(req.params.id, input)) });
  });
  app.delete('/api/routes/:id', (req, res) => {
    if (!profiles || typeof profiles.remove !== 'function') return res.status(503).json({ error: 'route-profile-store-unavailable' });
    if (activeProfile(directCliConfig, req.params.id)) throw requestError('route profile is used by an active direct CLI takeover', 'ROUTE_PROFILE_IN_USE', 409);
...
module.exports = { createWebApp, createWebServer, profileToView, redact, viewToProfile, validateProviderInput, validateRouteProfile, providerReferences };
