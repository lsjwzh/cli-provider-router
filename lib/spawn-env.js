...
// ── claude proxy env rewrite ─────────────────────────────────────────────────

// Rewrite a child process env so claude routes through the local claude-proxy
// (lib/proxy/claude.js) instead of the provider's real endpoint. Only applies to
...
// session route its subagents to cheaper providers. See lib/proxy/claude.js.
//
// Decoupled from multicc's session object: callers pass a `store` (or any
// object exposing getProvider(appType, id)) so this can resolve the provider
// without a module-level store singleton.
function applyClaudeProxyEnv(env, { providerId, sessionId, subagent, port, enabled, officialOAuth, store }) {
  if (!enabled) return;
  if (!providerId || !sessionId || !port) return;
  const p = store.getProvider('claude', providerId);
