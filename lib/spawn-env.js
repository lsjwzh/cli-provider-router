...
function resolveSpawnEnv({ cli, providerId, store, paths, cprHome, codexHomesDir }) {
  ...
  if (appType === 'claude') {
    ...
    return { env, skipDefaultModel: !!env.ANTHROPIC_BASE_URL, aliasOnly: !!env.ANTHROPIC_BASE_URL && !src.ANTHROPIC_MODEL, providerModel, providerModels, providerName: p.name, tools: src.CPR_TOOLS || src.MULTICC_TOOLS };
  }

  try {
    const resolvedPaths = ensureCprPaths(paths || createCprPaths({ home: cprHome }));
    const home = path.join(codexHomesDir || resolvedPaths.codexHomesDir || CODEX_HOMES_DIR, providerId);
