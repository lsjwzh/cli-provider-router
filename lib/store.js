...
function ensureDatabase() {
  ...
  console.log('[cli-provider-router] better-sqlite3 not installed — installing on demand (cc-switch import)…');
  try {
    // cwd is process.cwd() — the caller's project dir, NOT a hard-coded path.
    ...
    console.log('[cli-provider-router] better-sqlite3 installed and loaded');
    return true;
  } catch (e) {
    console.error('[cli-provider-router] Failed to install better-sqlite3:', e.message);
    return false;
  }
}
...
// diagnostics.
function createStore({ dataFile, ccSwitchDb } = {}) {
  const storeFile = dataFile || path.join(os.homedir(), '.cli-provider-router', 'providers.json');
  ...
  function saveStore(list) {
    try { fs.writeFileSync(storeFile, JSON.stringify(list, null, 2)); }
    catch (e) { console.error('[cli-provider-router] save providers.json failed:', e.message); }
  }
  ...
  function importFromCcSwitch() {
    ...
    if (!ensureDatabase()) {
      throw new Error(
        ...
        'Or manually: in the cli-provider-router install directory run `npm install better-sqlite3`'
      );
    }
