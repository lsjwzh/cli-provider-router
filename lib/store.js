...
function ensureDatabase() {
  return sqliteRuntimeStatus().available;
}
...
function summarize(p) {
  ...
  const protocol = p.appType === 'claude' ? 'anthropic' : 'openai';
  let wireApi = p.appType === 'claude' ? 'messages' : 'responses';
  if (p.appType === 'claude') {
    const env = cfg.env || {};
    ...
    wireApi = String(tomlValue(cfg.config, 'wire_api') || 'responses').toLowerCase();
...
// diagnostics.
function createStore({ dataFile, ccSwitchDb, cprHome, paths, referencesProvider } = {}) {
  // An absolute provider file is a complete host storage injection. Derive
  // descriptive paths beside it, but do not initialize the standalone CPR
  // tree (especially the default ~/.cli-provider-router) until a caller asks
  // for a standalone path explicitly.
  let resolvedPaths;
  if (paths) {
    resolvedPaths = ensureCprPaths(paths);
  } else if (dataFile && !cprHome) {
    const providerDir = path.dirname(path.resolve(dataFile));
    const inferredHome = path.basename(providerDir) === 'data'
      ? path.dirname(providerDir)
      : providerDir;
    resolvedPaths = createCprPaths({ home: inferredHome });
  } else {
    resolvedPaths = ensureCprPaths(createCprPaths({ home: cprHome }));
  }
  const storeFile = dataFile || resolvedPaths.providersFile;
  const ccDb = resolveCcDb(ccSwitchDb);
  let referenceResolver = typeof referencesProvider === 'function' ? referencesProvider : null;
  ...
  // Durable layer: schema envelope + rolling backups + fail-closed reads +
  // cross-process lock so the Web service and concurrent `cpr add/rm/import`
  // invocations never lose each other's updates. Legacy documents (bare array
  // or { providers: [...] }) migrate transparently on the next write.
  const durable = createDurableStore({
    file: storeFile,
    schemaName: 'cpr.providers',
    schemaVersion: 1,
    ...
    migrateLegacy(raw) {
      ...
      if (raw && typeof raw === 'object' && Array.isArray(raw.providers)) return raw.providers;
      return undefined;
    },
  });

  function loadStore() {
    return durable.load().payload;
  }
  ...
  function createProvider({ appType, name, baseUrl, authToken, model, models, useChatResponsesProxy, settingsConfig, aliasMap }) {
    ...
    durable.mutate(list => ({ next: [...list, p], result: null }));
  ...
  function updateProvider(appType, id, { name, baseUrl, authToken, model, models, useChatResponsesProxy, settingsConfig, aliasMap }) {
    return durable.mutate(list => {
      ...
      return { next: list, result: { id, appType } };
    }).result;
  }

  function deleteProvider(appType, id) {
    const references = referenceResolver ? referenceResolver(appType, id) : [];
    if (Array.isArray(references) && references.length) {
      const error = new Error(`provider is referenced by ${references.length} route profile(s)`);
      error.code = 'PROVIDER_IN_USE';
      error.statusCode = 409;
      error.references = references.map(profile => ({ id: profile.id, name: profile.name }));
      throw error;
    }
    const { changed } = durable.mutate(list => {
      const next = list.filter(p => !(p.appType === appType && p.id === id));
      return next.length === list.length ? null : { next, result: true };
    });
    return changed;
  }

  function setReferenceResolver(resolver) {
    if (resolver != null && typeof resolver !== 'function') throw new Error('provider reference resolver must be a function');
  ...
  function importFromCcSwitch() {
    ...
    const sqlite = sqliteRuntimeStatus();
    if (!sqlite.available) throw sqliteUnavailableError(sqlite.reason);
    const db = openSqliteDatabase(ccDb, { readonly: true, fileMustExist: true, timeout: 4000 });
    let rows;
    try {
    ...
    return durable.mutate(list => {
      ...
      return { next: list, result: { imported, updated, total: rows.length } };
    }).result;
  }
