...
function createRouteProfileStore(options = {}) {
  ...
  function create(input) {
    const profiles = list();
    const profile = normalizeRouteProfile(input);
  ...
  function update(id, patch) {
    ...
    profiles[index] = normalizeRouteProfile({ ...profiles[index], ...patch, id }, profiles[index]);
    save(profiles);
  ...
  return { list, get, create, update, remove, resolve, _dataFile: dataFile, _paths: paths };
}
