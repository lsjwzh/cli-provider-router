...
function createHandler(options = {}) {
  ...
  return async (req, res) => {
    ...
    const up = lib.request({
      ...
      upRes.once('aborted', () => emitUsageEvent({
        status: 'error', statusCode, isStream: true, errorCode: 'UPSTREAM_STREAM_ABORTED',
      }));
      upRes.once('error', () => emitUsageEvent({
        status: 'error', statusCode, isStream: true, errorCode: 'UPSTREAM_STREAM_FAILED',
      }));
