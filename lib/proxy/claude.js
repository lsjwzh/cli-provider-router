 ...
 */
function decodeCcfwModel(model, prefix = CCFW_PREFIX) {
  if (typeof model !== 'string' || !model.startsWith(prefix)) return null;
  const body = model.slice(prefix.length);
...
function mountClaudeProxy(app, options = {}) {
  app.use(options.mountPath || '/claude-proxy', createHandler(options));
}
