...
function maybeCapture(meta, headers, bodyBuf, captureDir = CAPTURE_DIR) {
  if (process.env.CPR_CAPTURE !== '1' && process.env.CCFW_CAPTURE !== '1') return;
  try {
    fs.mkdirSync(captureDir, { recursive: true });
    const fname = path.join(captureDir, `${meta.sessionId || 'nosess'}-${meta.stamp}.json`);
    let body;
  ...
  } catch (e) { console.log(`[cpr] capture failed: ${e.message}`); }
}
...
function createHandler(options = {}) {
  ...
  return async (req, res) => {
    ...
    maybeCapture({ sessionId, role, provider: creds.name || routeProviderId, url: `${base.origin}${fullPath}`, stamp: Date.now() }, safeHeaders, outBody, captureDir);
    ...
    const up = lib.request({
      ...
      if (!ok) {
        ...
        upRes.on('end', () => {
          ...
          if (usage && (typeof onUsage === 'function' || typeof onUsageEvent === 'function')) {
            const usageInfo = {
              sessionId, role,
            ...
            try {
              if (typeof onUsage === 'function') onUsage(usageInfo);
              if (typeof onUsageEvent === 'function') onUsageEvent({
                ...usageInfo,
...
module.exports = { mountClaudeProxy, createHandler, parseProxyUrl, decodeCcfwModel, decodeRoutedModel: decodeCcfwModel, resolveCreds, CPR_PREFIX, CCFW_PREFIX, LEGACY_MODEL_PREFIXES, ensureClaudeIdentity, readOfficialOAuthToken };
