  ...
  if (tee.isSSE) {
    ...
    while ((nl = tee.buf.indexOf('\n')) >= 0) {
      ...
      // ── onDelta sidecar: track the current content block and forward its deltas ──
        ...
        const delta = d.delta || {};
        let out = null;
        // text/thinking blocks stream .delta.text; tool_use streams .delta.partial_json
        if ((block.type === 'text' || block.type === 'thinking') && typeof delta.text === 'string') {
          out = block.type === 'thinking'
            ? { type: 'reasoning', text: delta.text }
