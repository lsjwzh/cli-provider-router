...
const { createRequestHooks, createRequestStage, readErrorBody } = require('./request-hook');
...
// Anthropic returns subscription usage in response HEADERS (not the SSE body):
//   anthropic-ratelimit-unified-5h-utilization / -reset / -status (+ 7d, overage)
// Only official-OAuth turns carry these; third-party relays do not, so a missing
// header => null => the host keeps its rate-limit bar hidden (never fabricated).
function extractRateLimit(headers) {
  const h = headers || {};
  const get = (k) => (h[k] != null ? String(h[k]) : null);
  const util5 = get('anthropic-ratelimit-unified-5h-utilization');
  if (util5 === null) return null;
  const utilization = Number(util5);
  if (!Number.isFinite(utilization)) return null;
  const resetRaw = get('anthropic-ratelimit-unified-5h-reset');
  const resetsAt = resetRaw !== null ? Number(resetRaw) : null;
  const rawStatus = get('anthropic-ratelimit-unified-5h-status');
  let status = 'allowed';
  if (rawStatus === 'rejected') status = 'rejected';
  else if (rawStatus && /warn|close|limit/i.test(rawStatus)) status = 'allowed_warning';
  const out = { rateLimitType: 'five_hour', status, utilization };
  if (resetsAt != null && Number.isFinite(resetsAt)) out.resetsAt = resetsAt;
  return out;
}
...
function newUsageTee() {
  return {
    ...
    isSSE: false,
    currentBlock: null, // last started block (fallback for compatible streams that omit index)
    contentBlocks: {},  // index -> {type, index, id, name}; supports interleaved blocks
  ...
  if (tee.isSSE) {
    ...
    while ((nl = tee.buf.indexOf('\n')) >= 0) {
      ...
      // ── onDelta sidecar: track the current content block and forward its deltas ──
        ...
        tee.currentBlock = block;
        if (block.index !== null) tee.contentBlocks[block.index] = block;
      } else if (d.type === 'content_block_delta' && tee.onDelta) {
        const block = Number.isInteger(d.index)
          ? tee.contentBlocks[d.index]
          : tee.currentBlock;
        if (!block) continue;
        const delta = d.delta || {};
        let out = null;
        ...
        if (block.type === 'text' && typeof delta.text === 'string') {
          ...
          out = { type: 'text', text: delta.content };
        } else if (delta.type === 'citations_delta' && delta.citation) {
          out = { type: 'source', source: delta.citation, blockId: block.id || '' };
        }
        if (out) { try { tee.onDelta(out, tee.deltaCtx); } catch (_) {} }
      } else if (d.type === 'content_block_stop') {
        if (Number.isInteger(d.index)) delete tee.contentBlocks[d.index];
        if (!Number.isInteger(d.index) || (tee.currentBlock && tee.currentBlock.index === d.index)) tee.currentBlock = null;
...
// Multi-account (host-injected): a provider record may pin ONE official account
// via settingsConfig.officialAccount.id (16-hex, issued by the host's account
// store). The marker reroutes the official branch to that account's credential
// through the injected readOfficialCredential — cpr itself stays ignorant of
// where account credentials live.
const OFFICIAL_ACCOUNT_ID_PATTERN = /^[a-f0-9]{16}$/;
function officialAccountIdFromProvider(provider) {
  ...
  const marker = cfg && cfg.officialAccount;
  const id = marker && typeof marker.id === 'string' ? marker.id : '';
  return OFFICIAL_ACCOUNT_ID_PATTERN.test(id) ? id : null;
}
 ...
 *  Accepts the legacy string keychain-service arg (shared login) or a context
 *  object {provider, providerId, accountId, keychainService} for account-marked
 *  providers. The default reader only knows the Keychain — an account context
 *  without a host-injected resolver fails LOUD, never borrowing the shared
 *  login for an account that isn't it. May return a Promise (host resolvers
 *  refresh-on-read). */
function readOfficialOAuthToken(arg = OAUTH_KEYCHAIN_SERVICE) {
  if (arg && typeof arg === 'object') {
    return { token: null, reason: `official account '${arg.accountId || '?'}' has no credential resolver — the host must inject readOfficialCredential` };
  }
  const o = readKeychainOAuth(arg);
...
function createHandler(options = {}) {
  const hooks = createRequestHooks(options);
  const {
    getProvider,
  ...
  return async (req, res) => {
    ...
    // Downstream liveness. The client here is a claude CLI process that can be
    // cancelled or killed at ANY moment — including while we are still buffering
    // its body or resolving the provider, not just mid-stream. Track that from
    // the earliest point that can lead to an upstream dial, so we never open (or
    // keep open) a provider connection for a request nobody is listening to.
    // `onClientClose` is installed further down, once there is a route to settle
    // and an upstream to tear down; until then this only records the fact.
    //
    // res 'close' is the signal, not req 'aborted': 'aborted' is deprecated, is
    // not emitted at all once the request body has been fully consumed (which is
    // exactly the window that matters here — the whole SSE turn), and every case
    // it covers also closes res. res 'close' fires for a killed process, a reset
    // socket, and a normal completion alike, so the handler is guarded below.
    let clientGone = false;
    let onClientClose = null;
    res.once('close', () => { clientGone = true; if (onClientClose) onClientClose(); });
    // Write-after-close on a dead socket surfaces as an 'error' on res. With no
    // listener Node promotes that to an unhandled 'error' event and takes the
    // host process down — an in-flight upRes.pipe(res) makes this reachable
    // whenever a client disconnects mid-stream. Nothing actionable is lost: the
    // peer is already gone, and the terminal event is emitted by onClientClose.
    res.on('error', () => {});
    ...
    let model = '';
    // `stream` is read here alongside `model` (one parse, not two) so a terminal
    // event raised BEFORE the upstream ever answers — a client that disconnects
    // during connect or before first byte — can still report isStream from what
    // the client actually asked for instead of guessing a constant.
    ...
    // Official (claude.ai OAuth subscription) route: the canonical
    // `claude-official` provider OR any provider marked with
    // settingsConfig.officialAccount.id (multi-account), in both cases only
    // when it has no stored baseUrl (a user could instead configure it with a
    // real API key, which the normal path handles). When the opt-in toggle is
    // on, replay the OAuth token to api.anthropic.com: the Keychain shared
    // login for the canonical provider, the host-resolved account credential
    // for marked ones. Scoped to these two shapes so other empty-baseUrl
    // providers still 502 rather than wrongly borrowing a subscription token.
    let officialOAuthToken = null;
    const officialAccountId = officialAccountIdFromProvider(provider);
    if ((routeProviderId === officialProviderId || officialAccountId) && (!creds || !creds.baseUrl)
        && process.env.CLAUDE_OFFICIAL_VIA_PROXY === '1') {
      const r = await readOfficialCredential(officialAccountId
        ? { provider, providerId: routeProviderId, accountId: officialAccountId, keychainService }
        : keychainService);
      if (r.token) {
    ...
    // ---- the host's local data-correction moment ----------------------------
    // Everything above this line decided WHERE the body goes; this is where the
    // host gets to look at the body itself, in the CLI's own protocol, before it
    // is forwarded — and (via onUpstreamRejected below) where it gets one
    // bounded look at a rejection. An untouched body is forwarded byte-for-byte,
    // so installing a hook costs an untouched turn nothing. See ./request-hook.
    ...
    if (await stage.prepare()) {
      console.log(`[cpr] onRequest hook rewrote the body sess=${sessionId || '-'} role=${role} provider=${routeProviderId} bytes=${outBody.length}->${Buffer.byteLength(stage.text)}`);
    }
    outBody = stage.buffer(outBody);
    ...
    // Built once, but re-used across attempts: everything here keyed off the
    // incoming request or the resolved credential, EXCEPT content-length, which
    // describes the body actually being sent and is re-derived per attempt by
    // logForward below (a repair hook can hand back a different body).
    ...
    // Per attempt, not per request: a retry after a host repair is its own
    // forward, and the log/capture must show the body that was actually sent —
    // including a content-length that matches it.
    const logForward = (attempt, bodyText) => {
      headers['content-length'] = String(Buffer.byteLength(bodyText));
      console.log(`[cpr] sess=${sessionId || '-'} role=${role} provider=${creds.name || routeProviderId} model=${finalModel || '(n/a)'}${attempt > 1 ? ` attempt=${attempt}` : ''} -> ${base.origin}${fullPath}`);
      ...
      maybeCapture({ sessionId, roleKind: roleContext.roleKind, agentRole: roleContext.agentRole || null, routeName: roleContext.routeName, provider: creds.name || routeProviderId, url: `${base.origin}${fullPath}`, stamp: Date.now() }, safeHeaders, bodyText, {
    ...
    // ---- terminal state -------------------------------------------------
    // ONE authority for "this turn reached a terminal state", deliberately
    // decoupled from the optional callbacks. Both emitters early-return when
    // their callback is absent or sessionId is empty, so `activityEnded` and
    // `usageEventEmitted` are NOT reliable terminal markers: in standalone use
    // (no onActivity wired) activityEnded stays false forever, and reading it to
    // answer "did this turn finish?" would report every NORMAL completion as a
    // client disconnect. settleTerminal is the guard; those two stay as inner
    // defence in depth so a callback still fires at most once.
    let terminalSettled = false;
    let observedStatusCode;   // undefined until the upstream responds
    let responseIsSSE = null; // null until upstream response headers are seen
    // isStream from what actually happened: the upstream response content-type
    // once headers exist, otherwise the client's own `stream` flag.
    const resolveIsStream = () => (responseIsSSE === null ? requestedStream : responseIsSSE);
    // Emits the usage event and the activity 'end' as one atomic pair, exactly
    // once per turn, whichever path gets here first. `status` stays inside the
    // existing success|error contract — a new terminal cause is expressed as an
    // errorCode on the usage event, never as a new status value and never as an
    // extra key on the activity payload (that one is a closed metadata set).
    const settleTerminal = (details = {}) => {
      if (terminalSettled) return;
      terminalSettled = true;
      emitUsageEvent({
        status: 'error',
        statusCode: observedStatusCode,
        isStream: resolveIsStream(),
        ...details,
      });
      emitActivity('end', { status: details.status === 'success' ? 'success' : 'error' });
    };

    let upstreamReq = null;   // http.ClientRequest, assigned right below
    let upstreamRes = null;   // http.IncomingMessage, assigned when headers land
    // Downstream died. Two jobs, in this order: converge the turn's terminal
    // state (so a host's per-provider producer accounting drains instead of
    // wedging on a request that will never report an end), then release the
    // provider socket (so we stop paying for a stream nobody reads — and so an
    // upstream that never ends does not hang for the life of the process).
    onClientClose = () => {
      // Normal completion also closes res. Every non-disconnect path has already
      // settled by then — our upRes 'end'/'aborted'/'error' listeners are all
      // registered BEFORE upRes.pipe(res), so they run before res is ended — and
      // writableEnded catches anything that finished writing without settling.
      if (terminalSettled || res.writableEnded) return;
      console.log(`[cpr] client closed early: sess=${sessionId || '-'} role=${role} provider=${creds.name || routeProviderId} — aborting upstream`);
      // Settle FIRST: our own destroy() below makes the upstream emit
      // 'aborted'/'error', and CLIENT_DISCONNECTED is the honest attribution.
      settleTerminal({ errorCode: 'CLIENT_DISCONNECTED' });
      try { if (upstreamRes) upstreamRes.destroy(); } catch (_) {}
      try { if (upstreamReq) upstreamReq.destroy(); } catch (_) {}
    };
    // The client died during setup (body upload / provider resolution), so res
    // 'close' already fired while onClientClose was still null. 'request' was
    // emitted, so the turn still has to reach a terminal state — but there is no
    // reason to dial the provider at all.
    if (clientGone) { onClientClose(); return; }

    // A non-2xx the host might be able to repair. The error body is drained and
    // offered to the hook; if it comes back with a repaired body we dial again,
    // otherwise the client gets exactly what it would have got without a hook.
    // Deliberately NOT awaited from the response callback: that one has to stay
    // synchronous, or a throw in here would surface as an unhandled rejection in
    // the host's process.
    const handleRejectedResponse = async (upRes, statusCode) => {
      let detail = '';
      try { detail = await readErrorBody(upRes); } catch (_) {}
      try {
        const retried = !clientGone && await stage.repair({
          status: statusCode, message: detail, attempt: dialAttempt, errorCode: 'UPSTREAM_HTTP_ERROR',
        });
        if (retried) {
          console.log(`[cpr] !! sess=${sessionId || '-'} role=${role} provider=${creds.name || routeProviderId} status=${statusCode} — retrying with a host-repaired body: ${detail.replace(/\s+/g, ' ').slice(0, 200)}`);
          dialUpstream();
          return;
        }
      } catch (e) {
        console.log(`[cpr] repair hook error: ${e.message}`);
      }
      if (clientGone) return; // terminal already settled by onClientClose
      const errHeaders = { ...upRes.headers };
      // The body was buffered (bounded) to be offered to the host, so the
      // upstream's own framing headers no longer describe what we send.
      delete errHeaders['content-length'];
      delete errHeaders['transfer-encoding'];
      try { res.writeHead(statusCode || 502, errHeaders); } catch (_) {}
      if (detail) console.log(`[cpr] !! sess=${sessionId || '-'} role=${role} status=${statusCode} body=${detail.replace(/\s+/g, ' ').slice(0, 500)}`);
      settleTerminal({ errorCode: 'UPSTREAM_HTTP_ERROR' });
      try { res.end(detail); } catch (_) {}
    };

    // One dial per attempt. A rejection the host repaired (onUpstreamRejected)
    // re-enters here with the repaired body; nothing downstream has been written
    // yet at that point, so the retry is invisible to the CLI.
    let dialAttempt = 0;
    const dialUpstream = () => {
      dialAttempt += 1;
      const wireBody = stage.buffer(outBody);
      logForward(dialAttempt, wireBody);
      ...
      up.on('error', (e) => {
        ...
        settleTerminal({ errorCode: 'UPSTREAM_CONNECT_FAILED' });
        // Our own destroy() on a client disconnect lands here as ECONNRESET. The
        // downstream socket is already gone, so there is nothing to answer — and
        // writing onto it would only risk a write-after-close.
    ...
    const handleUpstreamResponse = (upRes) => {
      upstreamRes = upRes;
      const statusCode = upRes.statusCode || 0;
      observedStatusCode = statusCode;
      responseIsSSE = String(upRes.headers['content-type'] || '').indexOf('text/event-stream') >= 0;
      // Tee subscription rate-limit headers (official-OAuth only). Best-effort:
      // a callback error never disturbs the forwarded stream (same guard as onUsage).
      if (typeof onRateLimit === 'function') {
        const rl = extractRateLimit(upRes.headers);
        if (rl) { try { onRateLimit({ sessionId, role, providerId: routeProviderId, providerName: creds.name || routeProviderId, rateLimitInfo: rl }); } catch (_) {} }
      }
      const ok = statusCode >= 200 && statusCode < 300;
      console.log(`[cpr] <- sess=${sessionId || '-'} role=${role} provider=${creds.name || routeProviderId} status=${statusCode}`);
      // Headers are deferred when the host may be able to repair the rejection:
      // a retried turn must not have told the client "failed" first.
      if (!ok && hooks.hasRejected) { void handleRejectedResponse(upRes, statusCode); return; }
      res.writeHead(statusCode || 502, upRes.headers);
      upRes.once('aborted', () => {
        settleTerminal({ errorCode: 'UPSTREAM_STREAM_ABORTED' });
      });
      upRes.once('error', () => {
        settleTerminal({ errorCode: 'UPSTREAM_STREAM_FAILED' });
      });
      const markFirstByte = () => {
      ...
      if (!ok) {
        ...
        upRes.on('end', () => {
          ...
          settleTerminal({ usage, status: 'success' });
        });
        upRes.pipe(res);
      }
    };
...
module.exports = { mountClaudeProxy, createHandler, parseProxyUrl, decodeCcfwModel, decodeRoutedModel: decodeCcfwModel, resolveCreds, maybeCapture, CPR_PREFIX, CCFW_PREFIX, LEGACY_MODEL_PREFIXES, ensureClaudeIdentity, readOfficialOAuthToken, officialAccountIdFromProvider,
