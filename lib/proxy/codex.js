...
function createCodexHandler(options = {}) {
  ...
  return async function handle(req, res, { providerId, sessionId = '', role = 'main' }) {
    ...
    if (target.mode === 'responses-compat') return proxyResponsesCompat(req, res, target, context, hooks);
    if (target.mode === 'direct-responses') return proxyResponsesDirect(req, res, target, context, hooks);
...
async function proxyResponsesDirect(req, res, target, context, hooks) {
  const stage = createRequestStage(hooks, {
...
async function proxyResponsesCompat(req, res, target, context, hooks) {
  const stage = createRequestStage(hooks, {
