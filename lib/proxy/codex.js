...
const { createProviderTransport } = require('./provider-transport');
...
// undici fetch failures surface as a bare TypeError "fetch failed" with the
// real reason (ENOTFOUND / ECONNREFUSED / ConnectTimeoutError / TLS errors)
// hidden on error.cause. Flatten the whole cause chain into the message so
// every downstream hop — codex's turn error, the host's chat bubble, the
// usage ledger — sees the root cause instead of the generic wrapper.
function describeTransportError(error) {
  const parts = [];
  const seen = new Set();
  for (let e = error; e && typeof e === 'object' && !seen.has(e) && seen.size < 5; e = e.cause) {
    seen.add(e);
    const name = e.name && e.name !== 'Error' ? `${e.name}: ` : '';
    if (e.code) parts.push(`${name}${e.code} ${e.message || ''}`.trim());
    else if (e.message) parts.push(name + String(e.message));
  }
...
function reportUsage(context, usage, isStream, outcome = {}) {
  ...
  //
  // The once-guard lives on the context rather than on onUsage/onActivity so a
  // request with no callbacks attached still converges to a single terminal
  // state. It also fixes the ordering that matters for client disconnects: the
  // disconnect settles first, and the loop-exit or catch-block report that the
  // abort provokes a moment later cannot overwrite the real cause.
  if (!context) return;
  if (context._terminalSettled) return;
  context._terminalSettled = true;
 ...
 */
// The routing context a hook needs, taken from the per-request `context` — the
// usage/activity callbacks in there are the router's own plumbing and have no
// business in a hook payload (see ./request-hook).
function hookContext(context, protocol, mode) {
  return {
    protocol,
    mode,
    providerId: context.providerId,
...
function createCodexHandler(options = {}) {
  ...
  const transport = options.providerTransport || createProviderTransport({
    env: options.env,
    onRotate: typeof options.onTransportRotate === 'function'
      ? options.onTransportRotate
      : event => console.warn(`[codex-proxy] provider dispatcher rotated: ${JSON.stringify(event)}`),
  });
  if (!transport || typeof transport.fetch !== 'function') {
    throw new TypeError('providerTransport.fetch is required');
  }
  const providerFetch = transport.fetch.bind(transport);
  const handle = async function handle(req, res, { providerId, sessionId = '', role = 'main' }) {
    ...
    if (target.mode === 'responses-compat') return proxyResponsesCompat(req, res, target, context, hooks, providerFetch);
    if (target.mode === 'direct-responses') return proxyResponsesDirect(req, res, target, context, hooks, providerFetch);

    // The host's local data-correction moment: hooks always see the CLIENT's
    // protocol (the Responses body the codex CLI sent), never the translated
    // chat body — the host repairs the shape it owns, and the translation runs
    // again on whatever comes back (see ./request-hook).
    const stage = createRequestStage(hooks, {
      ...hookContext(context, 'openai-responses', target.mode),
      isStream: true,
      body: req.body || {},
    });
    await stage.prepare();

    // Downstream liveness is registered before the upstream dial so a client that
    // gives up during connect is never answered by a request nobody is reading.
    const aborter = new AbortController();
    let clientClosed = false;
    const markClientClosed = () => {
      // res 'close' fires on a normal, fully written response too; writableEnded
      // is what separates that from a client that walked away mid-flight.
      if (clientClosed || res.writableEnded) return;
      clientClosed = true;
      reportUsage(context, null, true, { status: 'error', errorCode: 'CLIENT_DISCONNECTED' });
      // Breaking the read loop is not enough on its own: the undici body stays
      // unconsumed and the provider socket stays open (and billing). Abort it.
      try { aborter.abort(); } catch (_) {}
    };
    req.on('aborted', markClientClosed);
    res.on('close', markClientClosed);
    res.on('error', () => {});
    if (res.destroyed) { markClientClosed(); return; }

    // One dial per attempt; a rejection the host repaired re-enters the loop with
    // the repaired body, and nothing has been written downstream by then — the
    // SSE headers are only set once an attempt actually succeeds.
    for (let attempt = 1; ; attempt++) {
      let chatBody;
      try {
        chatBody = responsesToChat(stage.body || {});
      } catch (e) {
      ...
      let upstream;
      try {
        upstream = await providerFetch(target.url, {
          method: 'POST',
        ...
        sendFailed(res, `fetch upstream failed: ${describeTransportError(e)}`);
        return;
      }
      ...
      if (!upstream.ok || !upstream.body) {
        ...
        // The rejected body is offered to the host once (bounded): a repaired
        // history is retried instead of failing a turn the CLI cannot explain.
        if (!clientClosed && await stage.repair({
          status: upstream.status, message: detail, attempt, errorCode: 'UPSTREAM_HTTP_ERROR',
        })) {
          console.warn(`[codex-proxy] upstream ${upstream.status}; retrying with a host-repaired body: ${String(detail).replace(/\s+/g, ' ').slice(0, 200)}`);
          continue;
        }
      ...
      try {
        ...
        if (clientClosed) return; // terminal already settled by markClientClosed
        ...
        try {
          res.write(`event: response.failed\ndata: ${JSON.stringify({
            ...
            response: { status: 'failed', error: { message: 'stream error: ' + describeTransportError(e) } },
          })}\n\n`);
        } catch (_) {}
      ...
      // This attempt produced the answer; a rejection is the only way back round.
      return;
    }
  };
  Object.defineProperty(handle, 'close', {
    value: () => typeof transport.close === 'function' ? transport.close() : Promise.resolve(),
  });
  return handle;
}
...
function mountCodexProxy(app, options = {}) {
  ...
  return Object.freeze({ close: () => handle.close() });
}

async function proxyResponsesDirect(req, res, target, context, hooks, providerFetch) {
  const stage = createRequestStage(hooks, {
    ...hookContext(context, 'openai-responses', 'direct-responses'),
    isStream: !!(req.body && req.body.stream !== false),
    body: req.body || {},
  });
  await stage.prepare();
  // Per attempt: a repaired body changes what goes on the wire.
  const wireBody = () => ({ ...(stage.body || {}), stream: req.body && req.body.stream !== false });
  // Downstream liveness, registered before the upstream dial. Without it a client
  // that cancels mid-turn leaves the loop below draining the provider stream into
  // a socket nobody is on the other end of -- and if the provider itself stalls,
  // reader.read() never settles, so the request wedges and holds the provider
  // connection (and its billing) open indefinitely.
  const aborter = new AbortController();
  let clientClosed = false;
  // isStream is derived from what actually happened: the response content-type
  // once headers are in, falling back to what the client asked for when the
  // turn ends before the upstream ever answers.
  let responseIsSSE = null;
  const resolveIsStream = () => (responseIsSSE === null ? req.body && req.body.stream !== false : responseIsSSE);
  const markClientClosed = () => {
    // res 'close' fires on a normal, fully written response too; writableEnded is
    // what separates that from a client that walked away mid-flight.
    if (clientClosed || res.writableEnded) return;
    clientClosed = true;
    // Settle the terminal state before aborting: the abort rejects the pending
    // read, and the catch block below would otherwise bury the real cause under
    // UPSTREAM_STREAM_FAILED.
    reportUsage(context, null, resolveIsStream(), { status: 'error', errorCode: 'CLIENT_DISCONNECTED' });
    // Abandoning the reader is not enough -- the undici body stays unconsumed and
    // the provider socket stays open. Abort releases it.
    try { aborter.abort(); } catch (_) {}
  };
  req.on('aborted', markClientClosed);
  res.on('close', markClientClosed);
  ...
  // One dial per attempt; a rejection the host repaired re-enters with the
  // repaired body (headers are not written until an attempt answers 2xx).
  let upstream;
  for (let attempt = 1; ; attempt++) {
    try {
      upstream = await providerFetch(target.url, {
      ...
      reportUsage(context, null, false, { status: 'error', errorCode: 'UPSTREAM_CONNECT_FAILED' });
      return sendFailed(res, `fetch upstream failed: ${describeTransportError(error)}`);
    }
    if (!upstream.ok || !upstream.body) {
      const detail = await readErrorBody(upstream);
      context.statusCode = upstream.status;
  ...
  try {
    ...
    if (!res.writableEnded) sendFailed(res, `stream error: ${describeTransportError(error)}`);
    return;
  }
  try { res.end(); } catch (_) {}
}

async function proxyResponsesCompat(req, res, target, context, hooks, providerFetch) {
  const stage = createRequestStage(hooks, {
    ...hookContext(context, 'openai-responses', 'responses-compat'),
    isStream: true,
    body: req.body || {},
  });
  await stage.prepare();
  let body = { ...(stage.body || {}), stream: true };
  ...
  const markClientClosed = (reason) => {
    if (clientClosed || res.writableEnded) return;
    clientClosed = true;
    // This path already released the upstream via currentAborter. What it did not
    // do was say why the turn ended: the trailing reportUsage recorded a plain
    // 'success', so an abandoned turn was indistinguishable from a completed one.
    reportUsage(context, null, true, { status: 'error', errorCode: 'CLIENT_DISCONNECTED' });
  ...
  // Busy-retries and host repairs share this loop but not its budget: the extra
  // hookRetryMax iterations are exactly the repairs the stage will allow, so the
  // loop can never be pushed past what either bound permits.
  for (let attempt = 1; attempt <= COMPAT_BUSY_RETRY_MAX + hooks.retryMax; attempt++) {
    const stats = {
      attempt,
      events: 0,
      lines: 0,
    ...
    if (!upstream.ok || !upstream.body) {
      const detail = await readErrorBody(upstream);
      context.statusCode = upstream.status;
      // Nothing has been written downstream yet on this attempt (writes are
      // deferred until commit), so a repaired retry is invisible to the client.
      if (!clientClosed && await stage.repair({
        status: upstream.status, message: detail, attempt, errorCode: 'UPSTREAM_HTTP_ERROR',
      })) {
        console.warn(`[codex-proxy] [${reqId}] upstream ${upstream.status}; retrying with a host-repaired body: ${String(detail).replace(/\s+/g, ' ').slice(0, 200)}`);
        body = { ...(stage.body || {}), stream: true };
        continue;
      }
    ...
    try {
      ...
      const msg = describeTransportError(e);
      stats.readError = msg;
      if (!clientClosed) {
        commit();
        try {
