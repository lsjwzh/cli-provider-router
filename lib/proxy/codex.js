...
function reportUsage(context, usage, isStream) {
  if (!usage || !context || !context.sessionId || (typeof context.onUsage !== 'function' && typeof context.onUsageEvent !== 'function')) return;
  const usageInfo = {
  ...
  try {
    if (typeof context.onUsage === 'function') context.onUsage(usageInfo);
...
function createCodexHandler(options = {}) {
  const { getProvider, onUsage = options.usageSink, onUsageEvent } = options;
