...
const { normalizeProxyBaseUrl, openDatabase, readJson, statePath, writePrivateJson } = require('./common');
...
async function apply(options = {}) {
  ...
  const existing = readJson(statePath(options.home));
  if (existing && !['restored', 'full-restored'].includes(existing.status)) {
    ...
    if (current.takeover === 'active' && current.state.proxyBaseUrl === proxyBaseUrl) {
      if (existing.status !== 'active') {
        ...
        writePrivateJson(statePath(options.home), recovered);
  ...
  let plan;
  try { plan = collectTargets(source, proxyBaseUrl); } finally { source.close(); }
  ...
  const snapshotId = snap.manifest ? snap.manifest.snapshotId : snap.snapshotId;
  const pendingState = {
    version: 1,
    status: 'applying',
    snapshotId,
  ...
  writePrivateJson(statePath(options.home), pendingState);
  const live = openDatabase(info.dbPath, { fileMustExist: true });
  let result;
  try { result = mutate(live, plan.changes, 'apply'); } finally { live.close(); }
  const state = {
    version: 1,
    status: 'active',
    snapshotId,
  ...
  writePrivateJson(statePath(options.home), state);
  appendAudit(options.home, 'takeover.applied', { snapshotId: state.snapshotId, dbPath: info.dbPath, proxyBaseUrl, changed: result.changed });
