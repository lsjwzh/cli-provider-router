...
function requireDatabase() {
  if (!Database) {
    const error = new Error('CC-Switch takeover requires the optional dependency better-sqlite3');
...
function takeoverRoot(home) {
  return path.join(resolveHome(home), 'ccswitch');
}

function statePath(home) {
  return path.join(takeoverRoot(home), 'state.json');
}
...
function openDatabase(dbPath, options = {}) {
  ...
  return new DB(path.resolve(dbPath), options);
}
